...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['travel-agent', 'hotel-manager', 'customer-service-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ProviderInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/providers` : null),
    () => getProviders({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Providers',
      path: isTenantUser ? `/providers/view/${data?.[0]?.id}` : '/providers',
      entity: 'provider',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Bookings', path: '/bookings', entity: 'booking', service: AccessServiceEnum.PROJECT, icon: FiCalendar },
    { name: 'Hotels', path: '/hotels', entity: 'hotel', service: AccessServiceEnum.PROJECT, icon: FiHome },
    { name: 'Reviews', path: '/reviews', entity: 'review', service: AccessServiceEnum.PROJECT, icon: FiStar },
    { name: 'Rooms', path: '/rooms', entity: 'room', service: AccessServiceEnum.PROJECT, icon: FiLayout },

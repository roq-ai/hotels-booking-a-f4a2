...
import { getHotels, deleteHotelById } from 'apiSdk/hotels';
import { HotelInterface } from 'interfaces/hotel';

type ColumnType = ColumnDef<HotelInterface, unknown>;
...
export function HotelListPage(props: HotelListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getHotels({
        relations: ['provider', 'review.count', 'room.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['name.contains', 'address.contains', 'city.contains', 'country.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HotelInterface>>(
    () => `/hotels?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: HotelInterface) => {
    if (hasAccess('hotel', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/hotels/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('room', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'room',
          header: 'Room',
          accessorKey: 'room',
          cell: ({ row: { original: record } }: any) => record?._count?.room,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/hotels/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('hotel', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/hotels/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('hotel', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="hotel" />
        </Flex>

        {hasAccess('hotel', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/hotels/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}

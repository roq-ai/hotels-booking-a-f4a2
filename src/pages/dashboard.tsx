...
import { RoomListPage as ShowCaseList } from 'pages/rooms';

function DashboardPage() {
  const { session } = useSession();

  const hotelStat = useSWR<PaginatedInterface<HotelInterface>>(
    () => `/hotels`,
    () => getHotels({ limit: 1, offset: 0 }),
  );
  ...
  const roomStat = useSWR<PaginatedInterface<RoomInterface>>(
    () => `/rooms`,
    () => getRooms({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hotels',
      result: hotelStat?.data?.totalCount,
      isLoading: hotelStat.isLoading,
    ...
    {
      title: 'Rooms',
      result: roomStat?.data?.totalCount,
      isLoading: roomStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`rooms`} passHref legacyBehavior>
                  <Button

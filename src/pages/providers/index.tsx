...
type ColumnType = ColumnDef<ProviderInterface, unknown>;
...
export function ProviderListPage(props: ProviderListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ProviderInterface>>(
    () => `/providers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'mobile', header: 'Mobile', accessorKey: 'mobile' },
    ...
    hasAccess('hotel', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Hotel',
          accessorKey: 'hotel',
          cell: ({ row: { original: record } }: any) => record?._count?.hotel,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/providers/view/${record.id}`} passHref legacyBehavior>
                <Button
